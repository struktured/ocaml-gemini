...
(** Gemini configuration. These values are the only necessary parameters and
    secrets to connect to an endpoint.

    For each gemini environment a different configuration module should be
    instantiated. *)
module type S = sig
  val version : string
...
(** Makes a configuration module given a desired Gemini environment. It then
    infers the values from environment variables.
...
(** Creates a configuration module for the sandbox environment. Will query the
    unix environment for the necessary parameters.

    It is recommended to generate this module exactly once in your program. *)
module Sandbox () = struct
  include (val make "sandbox" : S)
end

(** Creates a configuration module for the production environment. Will query
    the unix environment for the necessary parameters.
...
(** Produces a configuration module given a string environment name which should
    be one of "production" or "sandbox". *)
let of_string s =
...
(** Ensures a configuration chosen given an optional configuration module,
    ...
    returns it unwrapped. Otherwise the unix environment is queried to produce a
